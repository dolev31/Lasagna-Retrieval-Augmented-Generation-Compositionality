...
\begin{abstract}
\noindent
We pre-register three confirmatory hypotheses that together ask
whether RAG pipelines can be designed compositionally:
\textbf{H1} asks whether the eight pipeline components combine
additively, \textbf{H2} asks whether a single balanced configuration
sweeps a wider region of the (faithfulness, latency, cost) tradeoff
envelope than a quality-only configuration, and \textbf{H3} asks
whether RAG profile rankings flip across a $40\times$ corpus
scale-up.
The pre-registered evaluation commits to a 242-configuration
D-optimal fractional factorial design spanning 8 pipeline dimensions,
5 benchmark datasets (2{,}300 queries per configuration), and a
three-track evaluation protocol covering retrieval, end-to-end
generation, and hallucination resistance.  To our knowledge, this is the first pre-registered
...
\paragraph{H3.}
Per-profile nonlinear least-squares fit of
$\text{NDCG@10}(N) = a \cdot N^b$ for each of the four operational
profiles (\textsc{Speed}, \textsc{Standard}, \textsc{Balanced},
